...
class LinzDistrictRegistryTest(unittest.TestCase):
    ...
    def testLinzDistrictRegistry(self):
        ...
        """
        layer = QgsVectorLayer(
            "Point?crs=EPSG:4326&field=fld1:string&field=fld2:string&field=type:string&field=estimated_pop:int&field=deprecated:int&field=stats_nz_pop:int&field=stats_nz_var_20:int&field=stats_nz_var_23:int&field=scenario_id:int&field=electorate_id_stats:string&field=expected_regions:int",
            "source", "memory")
        f = QgsFeature()
    ...
    def testVectorDistrictAtPoint(self):
        ...
        """
        layer = QgsVectorLayer(
            "Polygon?crs=EPSG:4326&field=fld1:string&field=type:string&field=estimated_pop:int&field=deprecated:int&field=stats_nz_pop:int&field=stats_nz_var_20:int&field=stats_nz_var_23:int&field=scenario_id:int&field=electorate_id_stats:string&field=expected_regions:int",
            "source", "memory")
        f = QgsFeature()
    ...
    def testCreateElectorate(self):
        ...
        self.assertEqual([f['expected_regions'] for f in layer.getFeatures()], [NULL, NULL, NULL, 1])
    ...
    def testUpdateStatsNzValues(self):
        ...
        reg.update_stats_nz_values(1, {'currentPopulation': 1111,
                                       'varianceYear1': 1.5,
                                       'varianceYear2': -1.1})
        self.assertEqual([f.attributes()[-6:-3] for f in layer.getFeatures()],
                         [[1111, 1.5, -1.1], [NULL, NULL, NULL], [NULL, NULL, NULL]])
        reg.update_stats_nz_values(2, {'currentPopulation': 1112,
                                       'varianceYear1': 1.6,
                                       'varianceYear2': -1.2})
        self.assertEqual([f.attributes()[-6:-3] for f in layer.getFeatures()], [[1111, 1.5, -1.1], [1112, 1.6, -1.2],
                                                                                [NULL, NULL, NULL]])
    ...
    def testGetStatsNzValues(self):
        ...
        self.assertEqual(reg.get_stats_nz_calculations(1),
                         {'currentPopulation': 1111, 'varianceYear1': 1.5, 'varianceYear2': -1.1})
